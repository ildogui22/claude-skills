...
Add a script to the project's existing task runner (`package.json` scripts, `Makefile`, `justfile`, `pyproject.toml`). The user should run `pnpm run <prototype-name>` or equivalent (in a `uv` project: a `Makefile` target or `uv run python <prototype>.py`) — never need to remember a path.
